    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-black text-brand-midnight tracking-tight">
          {profile?.role === 'INDIVIDUEL' ? 'Mes Résultats' : 'Diagnostics & Résultats'}
        </h1>
            ...
            <tbody className="divide-y divide-slate-50">
              {items.length > 0 ? items.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50/40 transition-all group">
                  ...
                  <td className="px-8 py-6">
                    <span className={`inline-flex items-center px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${d.status === 'Terminé' ? 'bg-emerald-50 text-emerald-600' : 'bg-brand-midnight text-white'
                      }`}>
                      {d.status}
                    </span>
                  </td>
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <Link to={`/diagnostic/result?id=${d.id}`} className="p-2.5 text-slate-300 hover:text-brand-midnight"><Eye size={18} /></Link>
                    </div>
                  </td>
                </tr>
              )) : !loading && (
